local wezterm = require 'wezterm'
local config = wezterm.config_builder()

-- Aesthetic & Font adjustments
config.color_scheme = 'catppuccin-mocha'
config.font = wezterm.font('CaskaydiaCove Nerd Font Mono')
config.font_size = 13.0
config.line_height = 1.1
config.window_background_opacity = 1.00

-- Default Shell Selection (Local Windows vs WSL)
config.default_domain = 'local'
config.default_prog = { 'pwsh.exe', '-NoLogo' }

-- Define your multi-environment profile entries
config.launch_menu = {
  {
    label = 'PowerShell',
    args = { 'pwsh.exe', '-NoLogo' },
  },
  {
    label = 'WSL (Linux Ubuntu)',
    args = { 'wsl.exe', '--distribution-id', '{b347e925-3e13-4df5-870d-073a96989d4a}', '--cd', '~' },
  },
  {
    label = 'WSL (Arch Linux)',
    args = { 'wsl.exe', '--distribution-id', '{415b4b2e-fb3c-4eb9-92e6-ba073c99785d}', '--cd', '~' },
  },
  {
    label = 'Git Bash',
    args = { 'C:\\Program Files\\Git\\bin\\bash.exe', '--login', '-i'},
  },
  {
    label = 'MSYS2 Environment',
    -- Note: Ensure you double-escape (\\) the Windows paths in Lua strings
    args = { 'C:\\msys64\\msys2_shell.cmd', '-defterm', '-no-start', '-ucrt64' },
  },
}

config.keys = {
  -- Bind vertical split to Ctrl+Shift+\ (example)
  {
    key = 'mapped:_',
    mods = 'CTRL|SHIFT',
    action = wezterm.action.SplitVertical { domain = 'CurrentPaneDomain' },
  },
  -- Bind horizontal split to Ctrl+Shift+H (example)
  {
    key = 'mapped:|',
    mods = 'CTRL|SHIFT',
    action = wezterm.action.SplitHorizontal { domain = 'CurrentPaneDomain' },
  },
}
--
-- Dynamic color scheme shifting based on active environment
-- wezterm.on('update-status', function(window, pane)
--     local overrides = window:get_config_overrides() or {}
--
--     -- Normalize the title to lowercase for easier matching
--     local title = pane:get_title():lower()
--     local domain = pane:get_domain_name()
--
--     -- Map your profiles to specific themes
--     if string.find(title, 'powershell') or string.find(title, 'pwsh') then
--         overrides.color_scheme = 'Campbell'
--     elseif string.find(title, 'msys') or string.find(title, 'bash') then
--         overrides.color_scheme = 'One Dark Pro'
--     elseif string.find(domain, 'WSL') or string.find(title, 'ubuntu') then
--         overrides.color_scheme = 'Tokyo Night'
--     else
--         -- Revert to default if switching to an unmapped profile
--         overrides.color_scheme = 'Tokyo Night'
--     end
--
--     -- Only apply the override if the theme has actually changed
--     local current_overrides = window:get_config_overrides() or {}
--     if current_overrides.color_scheme ~= overrides.color_scheme then
--         window:set_config_overrides(overrides)
--     end
-- end)

return config

--    "3024 (base16)",
--    "3024 (dark) (terminal.sexy)",
--    "3024 (light) (terminal.sexy)",
--    "3024 Day",
--    "3024 Day (Gogh)",
--    "3024 Night",
--    "3024 Night (Gogh)",
--    "3024Day (Gogh)",
--    "3024Night (Gogh)",
--    "Aardvark Blue",
--    "Abernathy",
--    "Aci (Gogh)",
--    "Aco (Gogh)",
--    "Adventure",
--    "Adventure Time (Gogh)",
--    "AdventureTime",
--    "Afterglow",
--    "Afterglow (Gogh)",
--    "Alabaster",
--    "Alien Blood (Gogh)",
--    "AlienBlood",
--    "Andromeda",
--    "Apathy (base16)",
--    "Apple Classic",
--    "Apple System Colors",
--    "Apprentice (Gogh)",
--    "Apprentice (base16)",
--    "Argonaut",
--    "Argonaut (Gogh)",
--    "Arthur",
--    "Arthur (Gogh)",
--    "Ashes (base16)",
--    "Ashes (dark) (terminal.sexy)",
--    "Ashes (light) (terminal.sexy)",
--    "Atelier Cave (base16)",
--    "Atelier Cave Light (base16)",
--    "Atelier Dune (base16)",
--    "Atelier Dune Light (base16)",
--    "Atelier Estuary (base16)",
--    "Atelier Estuary Light (base16)",
--    "Atelier Forest (base16)",
--    "Atelier Forest Light (base16)",
--    "Atelier Heath (base16)",
--    "Atelier Heath Light (base16)",
--    "Atelier Lakeside (base16)",
--    "Atelier Lakeside Light (base16)",
--    "Atelier Plateau (base16)",
--    "Atelier Plateau Light (base16)",
--    "Atelier Savanna (base16)",
--    "Atelier Savanna Light (base16)",
--    "Atelier Seaside (base16)",
--    "Atelier Seaside Light (base16)",
--    "Atelier Sulphurpool (base16)",
--    "Atelier Sulphurpool Light (base16)",
--    "AtelierSulphurpool",
--    "Atelierdune (dark) (terminal.sexy)",
--    "Atelierdune (light) (terminal.sexy)",
--    "Atelierforest (dark) (terminal.sexy)",
--    "Atelierforest (light) (terminal.sexy)",
--    "Atelierheath (dark) (terminal.sexy)",
--    "Atelierheath (light) (terminal.sexy)",
--    "Atelierlakeside (dark) (terminal.sexy)",
--    "Atelierlakeside (light) (terminal.sexy)",
--    "Atelierseaside (dark) (terminal.sexy)",
--    "Atelierseaside (light) (terminal.sexy)",
--    "Atlas (base16)",
--    "Atom",
--    "Atom (Gogh)",
--    "AtomOneLight",
--    "Aura (Gogh)",
--    "Aurora",
--    "Ayu Dark (Gogh)",
--    "Ayu Light (Gogh)",
--    "Ayu Mirage",
--    "Ayu Mirage (Gogh)",
--    "AyuDark (Gogh)",
--    "AyuLight (Gogh)",
--    "AyuMirage (Gogh)",
--    "Azu (Gogh)",
--    "Bamboo",
--    "Bamboo Light",
--    "Bamboo Multiplex",
--    "Banana Blueberry",
--    "Batman",
--    "Belafonte Day",
--    "Belafonte Day (Gogh)",
--    "Belafonte Night",
--    "Belafonte Night (Gogh)",
--    "BelafonteDay (Gogh)",
--    "BelafonteNight (Gogh)",
--    "Belge (terminal.sexy)",
--    "Bespin (base16)",
--    "Bespin (dark) (terminal.sexy)",
--    "Bespin (light) (terminal.sexy)",
--    "Bim (Gogh)",
--    "Birds Of Paradise (Gogh)",
--    "BirdsOfParadise",
--    "Bitmute (terminal.sexy)",
--    "Black Metal (Bathory) (base16)",
--    "Black Metal (Burzum) (base16)",
--    "Black Metal (Dark Funeral) (base16)",
--    "Black Metal (Gorgoroth) (base16)",
--    "Black Metal (Immortal) (base16)",
--    "Black Metal (Khold) (base16)",
--    "Black Metal (Marduk) (base16)",
--    "Black Metal (Mayhem) (base16)",
--    "Black Metal (Nile) (base16)",
--    "Black Metal (Venom) (base16)",
--    "Black Metal (base16)",
--    "Blazer",
--    "Blazer (Gogh)",
--    "Bleh-1 (terminal.sexy)",
--    "Blue Matrix",
--    "BlueBerryPie",
--    "BlueDolphin",
--    "Bluloco Light (Gogh)",
--    "Bluloco Zsh Light (Gogh)",
--    "BlulocoDark",
--    "BlulocoLight",
--    "BlulocoLight (Gogh)",
--    "BlulocoZshLight (Gogh)",
--    "Borland",
--    "Borland (Gogh)",
--    "Breath (Gogh)",
--    "Breath Darker (Gogh)",
--    "Breath Light (Gogh)",
--    "Breath Silverfox (Gogh)",
--    "BreathLight (Gogh)",
--    "BreathSilverfox (Gogh)",
--    "Breeze",
--    "Breeze (Gogh)",
--    "Brewer (base16)",
--    "Brewer (dark) (terminal.sexy)",
--    "Brewer (light) (terminal.sexy)",
--    "Bright (base16)",
--    "Bright Lights",
--    "Broadcast",
--    "Broadcast (Gogh)",
--    "Brogrammer",
--    "Brogrammer (Gogh)",
--    "Brogrammer (base16)",
--    "Brush Trees (base16)",
--    "Brush Trees Dark (base16)",
--    "Builtin Dark",
--    "Builtin Light",
--    "Builtin Pastel Dark",
--    "Builtin Solarized Dark",
--    "Builtin Solarized Light",
--    "Builtin Tango Dark",
--    "Builtin Tango Light",
--    "C64",
--    "C64 (Gogh)",
--    "CGA",
--    "CLRS",
--    "Cai (Gogh)",
--    "Calamity",
--    "Campbell (Gogh)",
--    "Canvased Pastel (terminal.sexy)",
--    "Catch Me If You Can (terminal.sexy)",
--    "Catppuccin Frappe",
--    "Catppuccin Frapp\u{e9} (Gogh)",
--    "Catppuccin Latte",
--    "Catppuccin Latte (Gogh)",
--    "Catppuccin Macchiato",
--    "Catppuccin Macchiato (Gogh)",
--    "Catppuccin Mocha",
--    "Catppuccin Mocha (Gogh)",
--    "Chalk",
--    "Chalk (Gogh)",
--    "Chalk (base16)",
--    "Chalk (dark) (terminal.sexy)",
--    "Chalk (light) (terminal.sexy)",
--    "Chalkboard",
--    "Chalkboard (Gogh)",
--    "ChallengerDeep",
--    "Chameleon (Gogh)",
--    "Chester",
--    "Ciapre",
--    "Ciapre (Gogh)",
--    "Circus (base16)",
--    "City Streets (terminal.sexy)",
--    "Classic Dark (base16)",
--    "Classic Light (base16)",
--    "Clone Of Ubuntu (Gogh)",
--    "CloneofUbuntu (Gogh)",
--    "Cloud (terminal.sexy)",
--    "Clrs (Gogh)",
--    "Cobalt 2 (Gogh)",
--    "Cobalt Neon",
--    "Cobalt Neon (Gogh)",
--    "Cobalt2",
--    "CobaltNeon (Gogh)",
--    "Codeschool (base16)",
--    "Codeschool (dark) (terminal.sexy)",
--    "Codeschool (light) (terminal.sexy)",
--    "Color Star (terminal.sexy)",
--    "Colorcli (Gogh)",
--    "Colorful Colors (terminal.sexy)",
--    "Colors (base16)",
--    "Count Von Count (terminal.sexy)",
--    "Crayon Pony Fish (Gogh)",
--    "CrayonPonyFish",
--    "Cupcake (base16)",
--    "Cupertino (base16)",
--    "Cyberdyne",
--    "DWM rob (terminal.sexy)",
--    "DanQing (base16)",
--    "DanQing Light (base16)",
--    "Darcula (base16)",
--    "Dark Ocean (terminal.sexy)",
--    "Dark Pastel",
--    "Dark Pastel (Gogh)",
--    "Dark Violet (base16)",
--    "Dark+",
--    "DarkPastel (Gogh)",
--    "Darkside",
--    "Darkside (Gogh)",
--    "Darktooth (base16)",
--    "Dawn (terminal.sexy)",
--    "DeHydration (Gogh)",
--    "Deafened (terminal.sexy)",
--    "Decaf (base16)",
--    "Default (dark) (terminal.sexy)",
--    "Default (light) (terminal.sexy)",
--    "Default Dark (base16)",
--    "Default Light (base16)",
--    "Dehydration (Gogh)",
--    "Derp (terminal.sexy)",
--    "Desert",
--    "Desert (Gogh)",
--    "Digerati (terminal.sexy)",
--    "Dimmed Monokai (Gogh)",
--    "DimmedMonokai",
--    "Dissonance (Gogh)",
--    "Django",
--    "DjangoRebornAgain",
--    "DjangoSmooth",
--    "Doom Peacock",
--    "DoomOne",
--    "DotGov",
--    "Dotshare (terminal.sexy)",
--    "Dracula",
--    "Dracula (Gogh)",
--    "Dracula (Official)",
--    "Dracula (base16)",
--    "Dracula+",
--    "Duotone Dark",
--    "ENCOM",
--    "Earthsong",
--    "Earthsong (Gogh)",
--    "Edge Dark (base16)",
--    "Edge Light (base16)",
--    "Ef-Arbutus",
--    "Ef-Autumn",
--    "Ef-Bio",
--    "Ef-Cherie",
--    "Ef-Cyprus",
--    "Ef-Dark",
--    "Ef-Day",
--    "Ef-Deuteranopia-Dark",
--    "Ef-Deuteranopia-Light",
--    "Ef-Duo-Dark",
--    "Ef-Duo-Light",
--    "Ef-Elea-Dark",
--    "Ef-Elea-Light",
--    "Ef-Frost",
--    "Ef-Kassio",
--    "Ef-Light",
--    "Ef-Maris-Dark",
--    "Ef-Maris-Light",
--    "Ef-Melissa-Dark",
--    "Ef-Melissa-Light",
--    "Ef-Night",
--    "Ef-Rosa",
--    "Ef-Spring",
--    "Ef-Summer",
--    "Ef-Symbiosis",
--    "Ef-Trio-Dark",
--    "Ef-Trio-Light",
--    "Ef-Tritanopia-Dark",
--    "Ef-Tritanopia-Light",
--    "Ef-Winter",
--    "Eighties (base16)",
--    "Eighties (dark) (terminal.sexy)",
--    "Eighties (light) (terminal.sexy)",
--    "Eldorado dark (terminal.sexy)",
--    "Elemental",
--    "Elemental (Gogh)",
--    "Elementary",
--    "Elementary (Gogh)",
--    "Elic (Gogh)",
--    "Elio (Gogh)",
--    "Embers (base16)",
--    "Embers (dark) (terminal.sexy)",
--    "Embers (light) (terminal.sexy)",
--    "Epiphany (terminal.sexy)",
--    "Eqie6 (terminal.sexy)",
--    "Equilibrium Dark (base16)",
--    "Equilibrium Gray Dark (base16)",
--    "Equilibrium Gray Light (base16)",
--    "Equilibrium Light (base16)",
--    "Erebus (terminal.sexy)",
--    "Espresso",
--    "Espresso (Gogh)",
--    "Espresso (base16)",
--    "Espresso Libre",
--    "Espresso Libre (Gogh)",
--    "EspressoLibre (Gogh)",
--    "Euphrasia (terminal.sexy)",
--    "Eva (base16)",
--    "Eva Dim (base16)",
--    "Everblush",
--    "Everblush (Gogh)",
--    "Everforest Dark (Gogh)",
--    "Everforest Light (Gogh)",
--    "EverforestDark (Gogh)",
--    "EverforestLight (Gogh)",
--    "Fahrenheit",
--    "Fairy Floss (Gogh)",
--    "Fairy Floss Dark (Gogh)",
--    "FairyFloss (Gogh)",
--    "FairyFlossDark (Gogh)",
--    "Fairyfloss",
--    "FarSide (terminal.sexy)",
--    "Fideloper",
--    "Firefly Traditional",
--    "FirefoxDev",
--    "Firewatch",
--    "FishTank",
--    "Fishbone (terminal.sexy)",
--    "Fishtank (Gogh)",
--    "Flat",
--    "Flat (Gogh)",
--    "Flat (base16)",
--    "Flat Remix (Gogh)",
--    "FlatRemix (Gogh)",
--    "Flatland",
--    "Flatland (Gogh)",
--    "Floraverse",
--    "ForestBlue",
--    "Foxnightly (Gogh)",
--    "Framer",
--    "Framer (base16)",
--    "Freya (Gogh)",
--    "FrontEndDelight",
--    "Frontend Delight (Gogh)",
--    "Frontend Fun Forrest (Gogh)",
--    "Frontend Galaxy (Gogh)",
--    "FrontendDelight (Gogh)",
--    "FrontendFunForrest (Gogh)",
--    "FrontendGalaxy (Gogh)",
--    "Fruit Soda (base16)",
--    "FunForrest",
--    "GJM (terminal.sexy)",
--    "Galaxy",
--    "Galizur",
--    "GeoHot (Gogh)",
--    "Geohot (Gogh)",
--    "Gigavolt (base16)",
--    "GitHub Dark",
--    "Github",
--    "Github (Gogh)",
--    "Github (base16)",
--    "Github Dark (Gogh)",
--    "Github Light (Gogh)",
--    "Glacier",
--    "Gnometerm (terminal.sexy)",
--    "Gogh (Gogh)",
--    "Gooey (Gogh)",
--    "Google (dark) (terminal.sexy)",
--    "Google (light) (terminal.sexy)",
--    "Google Dark (Gogh)",
--    "Google Dark (base16)",
--    "Google Light (Gogh)",
--    "Google Light (base16)",
--    "GoogleDark (Gogh)",
--    "GoogleLight (Gogh)",
--    "Gotham (Gogh)",
--    "Gotham (terminal.sexy)",
--    "Grandshell (terminal.sexy)",
--    "Grape",
--    "Grape (Gogh)",
--    "Grass",
--    "Grass (Gogh)",
--    "Grayscale (dark) (terminal.sexy)",
--    "Grayscale (light) (terminal.sexy)",
--    "Grayscale Dark (base16)",
--    "Grayscale Light (base16)",
--    "Green Screen (base16)",
--    "Greenscreen (dark) (terminal.sexy)",
--    "Greenscreen (light) (terminal.sexy)",
--    "Grey-green",
--    "Gruber (base16)",
--    "Gruvbox (Gogh)",
--    "Gruvbox Dark (Gogh)",
--    "Gruvbox Light",
--    "Gruvbox Material (Gogh)",
--    "Gruvbox dark, hard (base16)",
--    "Gruvbox dark, medium (base16)",
--    "Gruvbox dark, pale (base16)",
--    "Gruvbox dark, soft (base16)",
--    "Gruvbox light, hard (base16)",
--    "Gruvbox light, medium (base16)",
--    "Gruvbox light, soft (base16)",
--    "GruvboxDark",
--    "GruvboxDarkHard",
--    "GruvboxLight",
--    "Guezwhoz",
--    "HaX0R_BLUE",
--    "HaX0R_GR33N",
--    "HaX0R_R3D",
--    "Hacktober",
--    "Hardcore",
--    "Hardcore (Gogh)",
--    "Hardcore (base16)",
--    "Harmonic16 Dark (base16)",
--    "Harmonic16 Light (base16)",
--    "Harper",
--    "Harper (Gogh)",
--    "Heetch Dark (base16)",
--    "Heetch Light (base16)",
--    "Helios (base16)",
--    "Hemisu Dark (Gogh)",
--    "Hemisu Light (Gogh)",
--    "HemisuDark (Gogh)",
--    "HemisuLight (Gogh)",
--    "Highway",
--    "Highway (Gogh)",
--    "Hipster Green",
--    "Hipster Green (Gogh)",
--    "HipsterGreen (Gogh)",
--    "Hivacruz",
--    "Homebrew",
--    "Homebrew (Gogh)",
--    "Homebrew Light (Gogh)",
--    "Homebrew Ocean (Gogh)",
--    "Hopscotch",
--    "Hopscotch (base16)",
--    "Hopscotch.256",
--    "Horizon Bright (Gogh)",
--    "Horizon Dark (Gogh)",
--    "Horizon Dark (base16)",
--    "Horizon Light (base16)",
--    "HorizonBright (Gogh)",
--    "HorizonDark (Gogh)",
--    "Humanoid dark (base16)",
--    "Humanoid light (base16)",
--    "Hurtado",
--    "Hurtado (Gogh)",
--    "Hybrid",
--    "Hybrid (Gogh)",
--    "Hybrid (terminal.sexy)",
--    "IBM3270(HighContrast) (Gogh)",
--    "ICGreenPPL (Gogh)",
--    "ICOrangePPL (Gogh)",
--    "IC_Green_PPL",
--    "IC_Orange_PPL",
--    "IR Black (base16)",
--    "IR_Black",
--    "Ibm 3270 (High Contrast) (Gogh)",
--    "Ibm3270 (Gogh)",
--    "Ic Green Ppl (Gogh)",
--    "Ic Orange Ppl (Gogh)",
--    "Icy Dark (base16)",
--    "Idle Toes (Gogh)",
--    "IdleToes (Gogh)",
--    "Iiamblack (terminal.sexy)",
--    "Insignificato (terminal.sexy)",
--    "Invisibone (terminal.sexy)",
--    "Ir Black (Gogh)",
--    "IrBlack (Gogh)",
--    "Isotope (base16)",
--    "Isotope (dark) (terminal.sexy)",
--    "Isotope (light) (terminal.sexy)",
--    "Ivory Dark (terminal.sexy)",
--    "Ivory Light (terminal.sexy)",
--    "JWR dark (terminal.sexy)",
--    "Jackie Brown",
--    "Jackie Brown (Gogh)",
--    "JackieBrown (Gogh)",
--    "Japanesque",
--    "Japanesque (Gogh)",
--    "Jason Wryan (terminal.sexy)",
--    "Jellybeans",
--    "Jellybeans (Gogh)",
--    "JetBrains Darcula",
--    "Jup (Gogh)",
--    "Kanagawa (Gogh)",
--    "Kasugano (terminal.sexy)",
--    "Kibble",
--    "Kibble (Gogh)",
--    "Kimber (base16)",
--    "Kokuban (Gogh)",
--    "Kolorit",
--    "Konsolas",
--    "Lab Fox",
--    "Laser",
--    "Laserwave (Gogh)",
--    "Later This Evening",
--    "Later This Evening (Gogh)",
--    "LaterThisEvening (Gogh)",
--    "Lavandula",
--    "Lavandula (Gogh)",
--    "Light White (terminal.sexy)",
--    "Liquid Carbon (Gogh)",
--    "Liquid Carbon Transparent (Gogh)",
--    "LiquidCarbon",
--    "LiquidCarbonTransparent",
--    "LiquidCarbonTransparentInverse",
--    "London Tube (base16)",
--    "Londontube (dark) (terminal.sexy)",
--    "Londontube (light) (terminal.sexy)",
--    "Lost Woods (terminal.sexy)",
--    "Low Contrast (terminal.sexy)",
--    "Lumifoo (terminal.sexy)",
--    "Lunaria Dark (Gogh)",
--    "Lunaria Eclipse (Gogh)",
--    "Lunaria Light (Gogh)",
--    "LunariaDark (Gogh)",
--    "LunariaEclipse (Gogh)",
--    "LunariaLight (Gogh)",
--    "Macintosh (base16)",
--    "Maia (Gogh)",
--    "Man Page",
--    "Man Page (Gogh)",
--    "ManPage (Gogh)",
--    "Mar (Gogh)",
--    "Mariana",
--    "Marrakesh (base16)",
--    "Marrakesh (dark) (terminal.sexy)",
--    "Marrakesh (light) (terminal.sexy)",
--    "Mashup Colors (terminal.sexy)",
--    "Materia (base16)",
--    "Material",
--    "Material (Gogh)",
--    "Material (base16)",
--    "Material (terminal.sexy)",
--    "Material Darker (base16)",
--    "Material Lighter (base16)",
--    "Material Palenight (base16)",
--    "Material Vivid (base16)",
--    "MaterialDark",
--    "MaterialDarker",
--    "MaterialDesignColors",
--    "MaterialOcean",
--    "Mathias",
--    "Mathias (Gogh)",
--    "Matrix (terminal.sexy)",
--    "Medallion",
--    "Medallion (Gogh)",
--    "Mellow Purple (base16)",
--    "Mexico Light (base16)",
--    "Mikado (terminal.sexy)",
--    "Mikazuki (terminal.sexy)",
--    "Mirage",
--    "Misterioso",
--    "Misterioso (Gogh)",
--    "Miu (Gogh)",
--    "Mocha (base16)",
--    "Mocha (dark) (terminal.sexy)",
--    "Mocha (light) (terminal.sexy)",
--    "Modus-Operandi",
--    "Modus-Operandi-Deuteranopia",
--    "Modus-Operandi-Tinted",
--    "Modus-Operandi-Tritanopia",
--    "Modus-Vivendi",
--    "Modus-Vivendi-Deuteranopia",
--    "Modus-Vivendi-Tinted",
--    "Modus-Vivendi-Tritanopia",
--    "Molokai",
--    "Molokai (Gogh)",
--    "Mona Lisa (Gogh)",
--    "MonaLisa",
--    "Mono (terminal.sexy)",
--    "Mono Amber (Gogh)",
--    "Mono Cyan (Gogh)",
--    "Mono Green (Gogh)",
--    "Mono Red (Gogh)",
--    "Mono Theme (terminal.sexy)",
--    "Mono White (Gogh)",
--    "Mono Yellow (Gogh)",
--    "Monokai (base16)",
--    "Monokai (dark) (terminal.sexy)",
--    "Monokai (light) (terminal.sexy)",
--    "Monokai (terminal.sexy)",
--    "Monokai Dark (Gogh)",
--    "Monokai Pro (Gogh)",
--    "Monokai Pro Ristretto (Gogh)",
--    "Monokai Remastered",
--    "Monokai Soda",
--    "Monokai Soda (Gogh)",
--    "Monokai Vivid",
--    "MonokaiDark (Gogh)",
--    "MonokaiPro (Gogh)",
--    "MonokaiProRistretto (Gogh)",
--    "MonokaiSoda (Gogh)",
--    "Moonfly (Gogh)",
--    "Morada (Gogh)",
--    "Mostly Bright (terminal.sexy)",
--    "Muse (terminal.sexy)",
--    "N0Tch2K (Gogh)",
--    "N0tch2k",
--    "Nancy (terminal.sexy)",
--    "Nature Suede (terminal.sexy)",
--    "Navy and Ivory (terminal.sexy)",
--    "Nebula (base16)",
--    "Neon",
--    "Neon (terminal.sexy)",
--    "Neon Night (Gogh)",
--    "Neopolitan",
--    "Neopolitan (Gogh)",
--    "Nep (Gogh)",
--    "Neutron",
--    "Neutron (Gogh)",
--    "Night Owl (Gogh)",
--    "Night Owlish Light",
--    "NightLion v1",
--    "NightLion v2",
--    "NightOwl (Gogh)",
--    "Nightfly (Gogh)",
--    "Nightlion V1 (Gogh)",
--    "Nightlion V2 (Gogh)",
--    "NightlionV1 (Gogh)",
--    "NightlionV2 (Gogh)",
--    "Nighty (Gogh)",
--    "Nocturnal Winter",
--    "Nord (Gogh)",
--    "Nord (base16)",
--    "Nord Light (Gogh)",
--    "NordLight (Gogh)",
--    "Nova (base16)",
--    "Novel",
--    "Novel (Gogh)",
--    "Nucolors (terminal.sexy)",
--    "Nudge (terminal.sexy)",
--    "Numix Darkest (terminal.sexy)",
--    "Obsidian",
--    "Obsidian (Gogh)",
--    "Ocean",
--    "Ocean (base16)",
--    "Ocean (dark) (terminal.sexy)",
--    "Ocean (light) (terminal.sexy)",
--    "Ocean Dark (Gogh)",
--    "OceanDark (Gogh)",
--    "Oceanic Next (Gogh)",
--    "Oceanic-Next",
--    "OceanicMaterial",
--    "OceanicNext (Gogh)",
--    "OceanicNext (base16)",
--    "Ollie",
--    "Ollie (Gogh)",
--    "Omni (Gogh)",
--    "One Dark (Gogh)",
--    "One Half Black (Gogh)",
--    "One Light (Gogh)",
--    "One Light (base16)",
--    "OneDark (Gogh)",
--    "OneDark (base16)",
--    "OneHalfBlack (Gogh)",
--    "OneHalfDark",
--    "OneHalfLight",
--    "OneLight (Gogh)",
--    "Operator Mono Dark",
--    "Orangish (terminal.sexy)",
--    "Outrun Dark (base16)",
--    "Overnight Slumber",
--    "Oxocarbon Dark (Gogh)",
--    "PaleNightHC",
--    "Palenight (Gogh)",
--    "Pali (Gogh)",
--    "Panda (Gogh)",
--    "Pandora",
--    "Panels (terminal.sexy)",
--    "PaperColor Dark (base16)",
--    "PaperColor Light (base16)",
--    "PaperColorDark (Gogh)",
--    "PaperColorLight (Gogh)",
--    "Papercolor Dark (Gogh)",
--    "Papercolor Light (Gogh)",
--    "Paraiso (base16)",
--    "Paraiso (dark) (terminal.sexy)",
--    "Paraiso (light) (terminal.sexy)",
--    "Paraiso Dark",
--    "Paraiso Dark (Gogh)",
--    "ParaisoDark (Gogh)",
--    "Parker Brothers (terminal.sexy)",
--    "Pasque (base16)",
--    "Pastel White (terminal.sexy)",
--    "Paul Millr (Gogh)",
--    "PaulMillr",
--    "PaulMillr (Gogh)",
--    "Pencil Dark (Gogh)",
--    "Pencil Light (Gogh)",
--    "PencilDark",
--    "PencilLight",
--    "Peppermint",
--    "Peppermint (Gogh)",
--    "PhD (base16)",
--    "Phrak1 (terminal.sexy)",
--    "Piatto Light",
--    "Pico (base16)",
--    "Pixiefloss (Gogh)",
--    "Pnevma",
--    "Pnevma (Gogh)",
--    "Poimandres",
--    "Poimandres Storm",
--    "Pop (base16)",
--    "Popping and Locking",
--    "Porple (base16)",
--    "PowerShell (Gogh)",
--    "Powershell (Gogh)",
--    "Predawn (Gogh)",
--    "Pretty and Pastel (terminal.sexy)",
--    "Pro",
--    "Pro (Gogh)",
--    "Pro Light",
--    "Pulp (terminal.sexy)",
--    "Purple People Eater (Gogh)",
--    "Purple Rain",
--    "PurplePeopleEater (Gogh)",
--    "Purpledream (base16)",
--    "Qualia (base16)",
--    "Railscasts (base16)",
--    "Railscasts (dark) (terminal.sexy)",
--    "Railscasts (light) (terminal.sexy)",
--    "Rapture",
--    "Rasi (terminal.sexy)",
--    "Raycast_Dark",
--    "Raycast_Light",
--    "Rebecca (base16)",
--    "Red Alert",
--    "Red Alert (Gogh)",
--    "Red Phoenix (terminal.sexy)",
--    "Red Planet",
--    "Red Sands",
--    "Red Sands (Gogh)",
--    "RedAlert (Gogh)",
--    "RedSands (Gogh)",
--    "Relaxed",
--    "Relaxed (Gogh)",
--    "Retro",
--    "Rezza (terminal.sexy)",
--    "Rippedcasts",
--    "Rippedcasts (Gogh)",
--    "Ros\u{e9} Pine (Gogh)",
--    "Ros\u{e9} Pine (base16)",
--    "Ros\u{e9} Pine Dawn (Gogh)",
--    "Ros\u{e9} Pine Dawn (base16)",
--    "Ros\u{e9} Pine Moon (Gogh)",
--    "Ros\u{e9} Pine Moon (base16)",
--    "Ros\u{e9}Pine (Gogh)",
--    "Ros\u{e9}PineDawn (Gogh)",
--    "Ros\u{e9}PineMoon (Gogh)",
--    "Rouge 2",
--    "Royal",
--    "Royal (Gogh)",
--    "Rydgel (terminal.sexy)",
--    "Ryuuko",
--    "SOS (terminal.sexy)",
--    "Sagelight (base16)",
--    "Sakura",
--    "Sakura (base16)",
--    "Sandcastle (base16)",
--    "Sat (Gogh)",
--    "Scarlet Protocol",
--    "Sea Shells (Gogh)",
--    "SeaShells",
--    "Seafoam Pastel",
--    "Seafoam Pastel (Gogh)",
--    "SeafoamPastel (Gogh)",
--    "Selenized Dark (Gogh)",
--    "Selenized Light (Gogh)",
--    "Sequoia Monochrome",
--    "Sequoia Moonlight",
--    "Seti",
--    "Seti (Gogh)",
--    "Seti UI (base16)",
--    "Sex Colors (terminal.sexy)",
--    "Shades of Purple (base16)",
--    "Shaman",
--    "Shaman (Gogh)",
--    "Shapeshifter (base16)",
--    "Shapeshifter (dark) (terminal.sexy)",
--    "Shapeshifter (light) (terminal.sexy)",
--    "Shel (Gogh)",
--    "Shic (terminal.sexy)",
--    "Silk Dark (base16)",
--    "Silk Light (base16)",
--    "Simple Rainbow (terminal.sexy)",
--    "Slate",
--    "Slate (Gogh)",
--    "SleepyHollow",
--    "Smyck",
--    "Smyck (Gogh)",
--    "Snazzy",
--    "Snazzy (Gogh)",
--    "Snazzy (base16)",
--    "Soft Server (Gogh)",
--    "SoftServer",
--    "Solar Flare (base16)",
--    "Solar Flare Light (base16)",
--    "Solarized (dark) (terminal.sexy)",
--    "Solarized (light) (terminal.sexy)",
--    "Solarized Darcula",
--    "Solarized Darcula (Gogh)",
--    "Solarized Dark (Gogh)",
--    "Solarized Dark - Patched",
--    "Solarized Dark Higher Contrast",
--    "Solarized Dark Higher Contrast (Gogh)",
--    "Solarized Light (Gogh)",
--    "SolarizedDarcula (Gogh)",
--    "SolarizedDark (Gogh)",
--    "SolarizedDarkHigherContrast (Gogh)",
--    "SolarizedLight (Gogh)",
--    "Sonokai (Gogh)",
--    "SpaceGray",
--    "SpaceGray Eighties",
--    "SpaceGray Eighties Dull",
--    "SpaceGrayEighties (Gogh)",
--    "SpaceGrayEightiesDull (Gogh)",
--    "Spacedust",
--    "Spacedust (Gogh)",
--    "Spacegray (Gogh)",
--    "Spacegray Eighties (Gogh)",
--    "Spacegray Eighties Dull (Gogh)",
--    "Spacemacs (base16)",
--    "Spiderman",
--    "Splurge (terminal.sexy)",
--    "Spring",
--    "Spring (Gogh)",
--    "Square",
--    "Square (Gogh)",
--    "Srcery (Gogh)",
--    "Sublette",
--    "Subliminal",
--    "Summer Pop (Gogh)",
--    "Summerfruit Dark (base16)",
--    "Summerfruit Light (base16)",
--    "Sundried",
--    "Sundried (Gogh)",
--    "Swayr (terminal.sexy)",
--    "Sweet Eliverlara (Gogh)",
--    "Sweet Love (terminal.sexy)",
--    "Sweet Terminal (Gogh)",
--    "SweetTerminal (Gogh)",
--    "Symfonic",
--    "Symphonic (Gogh)",
--    "Synth Midnight Terminal Dark (base16)",
--    "Synth Midnight Terminal Light (base16)",
--    "SynthWave (Gogh)",
--    "Synthwave (Gogh)",
--    "Synthwave Alpha (Gogh)",
--    "SynthwaveAlpha",
--    "SynthwaveAlpha (Gogh)",
--    "Tango (base16)",
--    "Tango (terminal.sexy)",
--    "Tango Adapted",
--    "Tango Half Adapted",
--    "Tangoesque (terminal.sexy)",
--    "Tartan (terminal.sexy)",
--    "Teerb",
--    "Teerb (Gogh)",
--    "Tender (Gogh)",
--    "Terminal Basic",
--    "Terminal Basic (Gogh)",
--    "TerminalBasic (Gogh)",
--    "Terminix Dark (Gogh)",
--    "TerminixDark (Gogh)",
--    "Teva (terminal.sexy)",
--    "Thayer Bright",
--    "Thayer Bright (Gogh)",
--    "ThayerBright (Gogh)",
--    "The Hulk",
--    "Tin (Gogh)",
--    "Tinacious Design (Dark)",
--    "Tinacious Design (Light)",
--    "Tokyo Night",
--    "Tokyo Night (Gogh)",
--    "Tokyo Night Day",
--    "Tokyo Night Light (Gogh)",
--    "Tokyo Night Moon",
--    "Tokyo Night Storm",
--    "Tokyo Night Storm (Gogh)",
--    "TokyoNight (Gogh)",
--    "TokyoNightLight (Gogh)",
--    "TokyoNightStorm (Gogh)",
--    "Tomorrow",
--    "Tomorrow (Gogh)",
--    "Tomorrow (dark) (terminal.sexy)",
--    "Tomorrow (light) (terminal.sexy)",
--    "Tomorrow Night",
--    "Tomorrow Night (Gogh)",
--    "Tomorrow Night Blue",
--    "Tomorrow Night Blue (Gogh)",
--    "Tomorrow Night Bright",
--    "Tomorrow Night Bright (Gogh)",
--    "Tomorrow Night Burns",
--    "Tomorrow Night Eighties",
--    "Tomorrow Night Eighties (Gogh)",
--    "TomorrowNight (Gogh)",
--    "TomorrowNightBlue (Gogh)",
--    "TomorrowNightBright (Gogh)",
--    "TomorrowNightEighties (Gogh)",
--    "Toy Chest (Gogh)",
--    "ToyChest",
--    "Treehouse",
--    "Treehouse (Gogh)",
--    "Trim Yer Beard (terminal.sexy)",
--    "Twilight",
--    "Twilight (Gogh)",
--    "Twilight (base16)",
--    "Twilight (dark) (terminal.sexy)",
--    "Twilight (light) (terminal.sexy)",
--    "Ubuntu",
--    "UltraDark",
--    "UltraViolent",
--    "UnderTheSea",
--    "Unikitty",
--    "Unikitty Dark (base16)",
--    "Unikitty Light (base16)",
--    "Unikitty Reversible (base16)",
--    "Unsifted Wheat (terminal.sexy)",
--    "Ura (Gogh)",
--    "Urple",
--    "Urple (Gogh)",
--    "VSCodeDark+ (Gogh)",
--    "VSCodeLight+ (Gogh)",
--    "VWbug (terminal.sexy)",
--    "Vacuous 2 (terminal.sexy)",
--    "Vag (Gogh)",
--    "Vaughn",
--    "Vaughn (Gogh)",
--    "Vibrant Ink (Gogh)",
--    "VibrantInk",
--    "Vice Alt (base16)",
--    "Vice Dark (base16)",
--    "Violet Dark",
--    "Violet Light",
--    "VisiBlue (terminal.sexy)",
--    "VisiBone (terminal.sexy)",
--    "Visibone Alt. 2 (terminal.sexy)",
--    "Vs Code Dark+ (Gogh)",
--    "Vs Code Light+ (Gogh)",
--    "Warm Neon (Gogh)",
--    "WarmNeon",
--    "Website (Gogh)",
--    "Wez",
--    "Wez (Gogh)",
--    "Whimsy",
--    "Wild Cherry (Gogh)",
--    "WildCherry",
--    "Windows 10 (base16)",
--    "Windows 10 Light (base16)",
--    "Windows 95 (base16)",
--    "Windows 95 Light (base16)",
--    "Windows High Contrast (base16)",
--    "Windows High Contrast Light (base16)",
--    "Windows NT (base16)",
--    "Windows NT Light (base16)",
--    "Wombat",
--    "Wombat (Gogh)",
--    "Woodland (base16)",
--    "Wryan",
--    "Wryan (Gogh)",
--    "Wzoreck (Gogh)",
--    "X::DotShare (terminal.sexy)",
--    "X::Erosion (terminal.sexy)",
--    "XCode Dusk (base16)",
--    "Yousai (terminal.sexy)",
--    "Zenburn",
--    "Zenburn (Gogh)",
--    "Zenburn (base16)",
--    "aikofog (terminal.sexy)",
--    "alacritty",
--    "arcoiris",
--    "astromouse (terminal.sexy)",
--    "ayu",
--    "ayu_light",
--    "carbonfox",
--    "catppuccin-frappe",
--    "catppuccin-latte",
--    "catppuccin-macchiato",
--    "catppuccin-mocha",
--    "coffee_theme",
--    "cyberpunk",
--    "darkermatrix",
--    "darkmatrix",
--    "darkmoss (base16)",
--    "dawnfox",
--    "dayfox",
--    "deep",
--    "dirtysea (base16)",
--    "duckbones",
--    "duskfox",
--    "farmhouse-dark",
--    "farmhouse-light",
--    "flexoki-dark",
--    "flexoki-light",
--    "gooey (Gogh)",
--    "gotham (Gogh)",
--    "hardhacker",
--    "hund (terminal.sexy)",
--    "iTerm2 Dark Background",
--    "iTerm2 Default",
--    "iTerm2 Light Background",
--    "iTerm2 Pastel Dark Background",
--    "iTerm2 Smoooooth",
--    "iTerm2 Solarized Dark",
--    "iTerm2 Solarized Light",
--    "iTerm2 Tango Dark",
--    "iTerm2 Tango Light",
--    "ibm3270 (Gogh)",
--    "iceberg-dark",
--    "iceberg-light",
--    "idea",
--    "idleToes",
--    "jmbi (terminal.sexy)",
--    "jubi",
--    "kanagawa (Gogh)",
--    "kanagawabones",
--    "kokuban (Gogh)",
--    "laserwave (Gogh)",
--    "lovelace",
--    "matrix",
--    "midnight-in-mojave",
--    "mono-amber (Gogh)",
--    "mono-cyan (Gogh)",
--    "mono-green (Gogh)",
--    "mono-red (Gogh)",
--    "mono-white (Gogh)",
--    "mono-yellow (Gogh)",
--    "neobones_dark",
--    "neobones_light",
--    "neon-night (Gogh)",
--    "nightfox",
--    "nighty (Gogh)",
--    "niji",
--    "nord",
--    "nord-light",
--    "nordfox",
--    "palenight (Gogh)",
--    "pinky (base16)",
--    "primary",
--    "purplepeter",
--    "rebecca",
--    "rose-pine",
--    "rose-pine-dawn",
--    "rose-pine-moon",
--    "s3r0 modified (terminal.sexy)",
--    "seoulbones_dark",
--    "seoulbones_light",
--    "shades-of-purple",
--    "summer-pop (Gogh)",
--    "summercamp (base16)",
--    "sweet-eliverlara (Gogh)",
--    "synthwave",
--    "synthwave-everything",
--    "tender (base16)",
--    "terafox",
--    "theme2 (terminal.sexy)",
--    "thwump (terminal.sexy)",
--    "tlh (terminal.sexy)",
--    "tokyonight",
--    "tokyonight-day",
--    "tokyonight-storm",
--    "tokyonight_day",
--    "tokyonight_moon",
--    "tokyonight_night",
--    "tokyonight_storm",
--    "vimbones",
--    "vulcan (base16)",
--    "wilmersdorf",
--    "zenbones",
--    "zenbones_dark",
--    "zenbones_light",
--    "zenburn (terminal.sexy)",
--    "zenburned",
--    "zenwritten_dark",
--    "zenwritten_lightu,
